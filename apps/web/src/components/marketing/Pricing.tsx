    ...
    <section id="pricing" className="max-w-6xl mx-auto px-6 py-24">
      {heading && (
        <div className="max-w-2xl mx-auto text-center">
          <span className="text-xs font-semibold uppercase tracking-wider text-clay">Pricing</span>
          ...
          return (
            <div
              ...
              className={`relative flex flex-col rounded-3xl border bg-card p-7 ${
                popular ? "border-clay/40 shadow-lift md:-translate-y-2" : "border-line shadow-soft"
              }`}
            >
              {popular && (
              ...
              <p className="text-sm text-muted mt-1 min-h-[40px]">{plan.tagline}</p>

              <div className="mt-5 flex items-baseline gap-1">
                <span className="font-serif text-4xl text-ink">{formatPrice(plan.id, currency)}</span>
      ...
      <p className="text-center text-xs text-faint mt-10">
        Prices shown in {currency}. Charged in your local currency at checkout. Need something custom?{" "}
        <a href="mailto:sales@upstep.dev" className="text-clay hover:underline">
          Talk to us
        </a>
        .
      </p>
